...
class Table:
    ...
    def write_to_tailPage(self, record, schema_encoding):
        ...
        if self.last_TID_used == -1 or not self.check_page_space(record.rid):
            ...
            print("Recycling recently used tail page\n")
        ...
        i = INIT_COLS
        while i < (INIT_COLS + self.num_columns):
            print("Table.write_to_tailPage() - Accessing Page i=", i, " and Record.column=", i-INIT_COLS, "\n")
            if record.columns[i-INIT_COLS] is not None:
                print("To be written val: ", record.columns[i-INIT_COLS], "\n")
                cur_record_pages[i].write(record.columns[i-INIT_COLS])
            else: # Write default values
                print("NONE (0) to be written\n")
        ...
        for baseID in range(self.LID_counter):
            ...
            if base_key == record.key:
               ...
               break
        '''
        print("Now writing to metadata columns: Indireciton, RID, Time, and Schema\n")
        ...
        try:
            ...
            print("Updating indirection column\n")
    ...
    def collect_values(self, start_range, end_range, col_index):
       ...
       for key_val in range(start_range, end_range):
           rid_batch = self.indexer.locate(key_val)
           if len(rid_batch): # At least one matching RID found
              rid_list += rid_batch # Combine into one list
       ...
       bools = list(map(check_equality, rid_list, latest_rids))
