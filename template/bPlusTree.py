# ADAPTED FROM https://github.com/NicolasLM/bplustree
...
class BPlusTree:
    ...
    def insert(self, key, value: bytes, replace=False):
        ...
        :param key: The key at which the value will be recorded
        ...
        # Have to modify so that entry class is not used
        '''
        try:
        ...
        '''
        data = (key, value)
        # if node.can_add_entry is false, then the node will hit maximum capacity on its next insert
        if node.can_add_entry:
            node.insert_entry(data)
    ...
    def _search_in_tree(self, key, node) -> 'Node':
        # Find the right node and return it
